...
for(i in 1:5){
  j = j + i
  result1[i] = 1-(j/(1+2+3+4+5))
  print(result1)
}
result1

F_level <- quantile(cust_df$Freq, probs = result1)
M_level <- quantile(cust_df$Money, probs = result1)

cust_df <- cust_df %>%
  mutate(R_score = case_when(.$Recency_days <= R_level[1] ~ 5,
                             .$Recency_days <= R_level[2] ~ 4,
                             .$Recency_days <= R_level[3] ~ 3,
                             .$Recency_days <= R_level[4] ~ 2,
                             TRUE ~ 1),
